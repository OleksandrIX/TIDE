...
def create_csv(root_dir, train_validation_split=0.8):
    ...
    for lang in languages:
        logger.debug("Processing language: {}".format(lang))
    ...
    logger.debug("Smallest count: {}".format(min(counter.values())))
    ...
    for lang in languages:
        ...
        test_set += zip(file_names[lang][num_train + num_validation:num_train + num_validation + num_test],
    ...
    # Stats
    logger.debug("[Training]   files per language: {} Total: {}".format(num_train, num_train * len(languages)))
    logger.debug("[Validation] files per language: {} Total: {}".format(num_validation, num_validation * len(languages)))
    logger.debug("[Testing]    files per language: {} Total: {}".format(num_test, num_test * len(languages)))
