...
def neg23File(filePath):
    if notAudio(filePath):
        logger.warning("Not a valid audio file.")
        return False
    logger.debug("Scanning " + filePath + " for loudness...")
    try:
        loudnessStats = r128Stats(filePath)
    except:
        logger.exception("neg23 encountered an error scanning " + filePath)
    gainAmount = linearGain(loudnessStats['I'])
    logger.info("Creating -23LUFS file at " + filePath)
    try:
        # applying the normalisation and saving it to the same place as it was (replacing)
        ffApplyGain(filePath, filePath, gainAmount)
    except:
        logger.exception("neg23 encountered an error applying gain to " + filePath)
    logger.debug("Done with " + filePath)
