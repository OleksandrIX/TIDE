...
def train(cli_args, log_dir):
    ...
    if config is None:
        logger.warning("Please provide a config.")
    ...
    # Do evaluation on model with the best validation accuracy
    best_epoch = np.argmax(history.history["val_acc"])
    logger.info("Log files: {}".format(log_dir))
...
if __name__ == "__main__":
    ...
    logger.add(os.path.join(log_dir, "train.log"))
