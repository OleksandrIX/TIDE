...
def predict(model, input_file, config):
    config = load(open(config, "rb"), Loader=FullLoader)
    class_labels = config["label_names"]

    # the file is not normalised before predicting in this script
    params = {"pixel_per_second": config["pixel_per_second"], "input_shape": config["input_shape"],
    ...
    logger.debug(classes, class_labels[average_class], average_prob)
