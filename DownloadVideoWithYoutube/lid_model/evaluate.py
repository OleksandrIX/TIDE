...
def metrics_report(y_true, y_pred, probabilities, label_names=None):
    ...
    logger.info("Equal Error Rate (avg) %s" % equal_error_rate(y_true, probabilities))
    logger.debug(classification_report(y_true, y_pred, labels=available_labels, target_names=label_names))
    logger.debug(confusion_matrix(y_true, y_pred, labels=available_labels))


def evaluate(args):
    config = load(open(cli_args.config, "rb"), Loader=FullLoader)
